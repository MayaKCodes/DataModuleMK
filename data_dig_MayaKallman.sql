...
SELECT gender,COUNT(*) AS "hero_count", ROUND(AVG(intelligence)) AS "avg_intelligence", ROUND(AVG(strength)) AS "avg_strength", ROUND (AVG(speed)) AS "avg_speed", ROUND(AVG(height_m)) AS "avg_height" FROM marvels
    GROUP BY gender;

SELECT name, MAX(height_m) FROM marvels;
SELECT name, MAX(strength) FROM marvels;
SELECT name, MAX(intelligence) FROM marvels;
SELECT name, MAX(popularity), alignment FROM marvels;

SELECT name, MAX(popularity) AS "popularity_score", alignment FROM marvels
    GROUP BY alignment;

SELECT name, MAX(popularity) AS "popularity_score", alignment FROM marvels
    GROUP BY gender;

SELECT gender, ROUND(AVG(popularity)) from marvels
    GROUP BY gender;
    
SELECT hometown, COUNT(hometown) AS "number_heros_from_here" FROM marvels
    GROUP BY hometown;
    
SELECT name, speed, strength FROM marvels WHERE speed>3 AND strength>5;

SELECT name, alignment, popularity,
    CASE
    WHEN popularity>50 THEN "cool"
    WHEN popularity >10 THEN "mid"
    ELSE "lame"
    END AS "rating"
    FROM marvels;
